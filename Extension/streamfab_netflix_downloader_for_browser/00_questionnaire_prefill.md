...
2. 一句话定位（已约定）
- 面向 Disney?/Netflix? 用户的专用浏览器下载插件，仅支持 Netflix 站点视频下载。

3. 核心用户（ytdlp兜底，待确认）
- 有离线观看需求的流媒体用户
- 已订阅或有试用意愿的 Netflix 用户

4. 核心功能清单（已约定）
- 仅支持 Netflix 站点
- 支持 Netflix DRM 下载（产品定义范围内）
- 批量发起任务 + 串行下载（并发1）
- 下载配置基于 meta 返回（5项）
- CoApp 协同下载执行与状态回传
...
3. 设置结构（已约定）
- Extension + CoApp
- 路径：VIP Services -> Preferred Settings / Preferred Settings-Netflix
