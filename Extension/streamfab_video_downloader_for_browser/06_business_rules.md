...
## 定价与套餐
- 免费版：无永久免费版。
- 试用版（Free Trial）：默认状态，提供 5 次全功能试用配额。
- 订阅版（Premium）：年费 / 终身 / Fab365。

## 权益总览（Trial vs Premium）
...
- 音频下载：支持（最高 320kbps）。
- 批量下载：支持（Trial 受配额限制）。
- 并发任务（系统级）：最多 5 个 Downloading/Turbo 并发，超出进入 Pending。
...
- 重置规则：Member Center 提供每年有限次数（通常 1 次）的 `Deauthorize All`。
...
- Turbo 规则：
  ...
  - Turbo 本身不产生独立扣点，不与下载扣点做叠加计算。
  - Trial 用户是否可用 Turbo 由剩余试用次数决定：无剩余次数时，Turbo 不可用。

## 订阅状态与阻断提示
- 订阅过期：回退到试用结束状态，触发 `Subscription Expired`，仅保留 Always Free 权益。
- 订阅有效但当日额度为 0：点击下载时弹窗：
...
- 规则：
  1. 任务创建并通过校验后，先预占额度（Reserve -1）。
  2. 任务 `Completed` 时，预占转永久扣减（Commit）。
  3. 任务 `Failed` 或 `Canceled` 时，自动返还预占（Release +1）。
  4. `Retry` 属于同一任务延续，不重复预占。
- 批量/剧集：按可创建任务数预占；成功项确认扣减，失败/取消项返还。

## 退款与异常补偿
- 自动返还：所有非成功结束状态（Failed/Canceled）自动返还预占配额。
- 人工补偿：系统故障导致返还异常时，用户可联系客服人工补发。
- 订阅退款：遵循 StreamFab 官网标准退款政策，插件端不处理。
