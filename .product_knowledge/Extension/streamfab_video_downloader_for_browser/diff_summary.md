...
> 关联：[`_common/READING_MAP.md`](../_common/READING_MAP.md)、[`_common/references/baselines/ytdlp_mode.md`](../_common/references/baselines/ytdlp_mode.md)
...
| 检测策略 | 预分析 + 多 Origin 探测；Detected 以 Origin 为单位隔离 | _common/references/baselines/ytdlp_mode.md § 二 |
| 下载并发 | 5（超过进入 Pending） | _common/references/baselines/ytdlp_mode.md § 三 |
| Always Free 路径 | YouTube < 720p + 关 Turbo = `0` 点；仅官网 / Edge 版可用 | _common/references/baselines/ytdlp_mode.md § 六 |
| Turbo-Speed 开关 | 付费权益，订阅默认可用；Trial 无剩余次数时不可用；本身不产生扣点 | requirements/plugin_requirement.md § 视频下载 - 配置参数 |
| 渠道差异 | Chrome 商店版不支持 YouTube 下载（政策）；Edge 商店 + 官网完整 | _common/references/platform_diffs.md § 三 |
...
| DRM 边界 | 不下载 DRM；显示 `Protected Video Content`，引导桌面客户端 | _common/references/tech_limits.md § 四 |
| 历史记录上限 | Downloaded 页保留最近 20 条 | _common/references/tech_limits.md § 六 |
| 展示模式 | 支持 Popup + Sidebar 双模式，共享同一套主框架 | _common/references/baselines/ytdlp_mode.md § 九 |
...
| Detected 状态机 | 状态多于 netflix_mode：检测中 / 新增加载中 / 空状态 / 未登录 / 未装 CoApp / CoApp 更新 / DRM 提示 / YouTube 限制（Chrome） | _common/references/baselines/ytdlp_mode.md § 二 |
