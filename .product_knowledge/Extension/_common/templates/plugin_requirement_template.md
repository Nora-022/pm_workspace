...
> **写作定位**：此节是平台简介，让读者在 30 秒内建立对站点的基本认知。技术性调研内容（账号/登录限制、播放协议、DRM、清晰度、对插件需求字段的影响、多站点差异等）一律写入 `requirements/site_research_notes.md`，本节末尾附链接指向。
> 
...
> - **调研笔记链接**：附 `requirements/site_research_notes.md`（多站点情况逐站列出对应锚点）。
> 
...
> **填写规则**：差异时填，与基线（_common/06_business_rules.md）一致时整块删除。
